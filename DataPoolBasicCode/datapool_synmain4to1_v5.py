...
@author: Cheng Yuxuan Original, Synthesis 4-to-1 Version 5 is discarded, due to old implementation. 2023/09/07
"""
import os
import cv2
# import csv
...
from datapool_synthauxiliary_v5 import (correct_distortion_patch1, correct_distortion_patch2, 
                                        correct_distortion_patch3, correct_distortion_patch4, 
                                        analyze_pattern, splicing_str, shrink_object, 
                                        check_objectsize, check_objectorient, xml_label_generate, 
                                        multiply_ratio_to_list, add_offset_to_list, add_pad_to_list)
...
class four_to_one_less_config1_search:
    ...
    def do_search(self, save_dir, scope):
        ...
        for i in range(len(list_target)):
            ...
            list2[10] = float(list2[10]) # area object
            list2[11] = float(list2[11]) # area cutout
            list2[12] = float(list2[12]) # cutout/object
            
            # 当图块中多个目标的坐标不为0时拼接字符, 13为多目标坐标
            if list2[13] != str(0):
                temp = list2[13]
                for i in range(14, len(list2)):
                    temp = temp + list2[i]
                list2[13] = literal_eval(temp) # 处理成字符串化列表
                list2[13] = splicing_str(list2[13]) # 拼接成原坐标列表
        ...
        if len(self.potential_area_1) != 0 and len(self.potential_area_2) != 0 and len(self.potential_area_3) != 0 and len(self.potential_area_4) != 0:
            ...
            while(len(self.potential_area_1) != 0 or len(self.potential_area_2) != 0 or len(self.potential_area_3) != 0 or len(self.potential_area_4) != 0):
                ...
                lab1_multi, lab2_multi, lab3_multi, lab4_multi = [], [], [], []
                
                # 标签计算为实际目标坐标减去切割区域坐标
                img1 = img1[self.i[7]:self.i[9], self.i[6]:self.i[8]]
                lab1_whole = [self.i[1]-self.i[6], self.i[2]-self.i[7], self.i[3]-self.i[6], self.i[4]-self.i[7], self.i[5]]
                for n in range(len(self.i[13])):
                    lab1_multi.append([self.i[13][n][0]-self.i[6], self.i[13][n][1]-self.i[7], self.i[13][n][2]-self.i[6], self.i[13][n][3]-self.i[7], self.i[13][n][4]])
                
                img2 = img2[self.j[7]:self.j[9], self.j[6]:self.j[8]]
                lab2_whole = [self.j[1]-self.j[6], self.j[2]-self.j[7], self.j[3]-self.j[6], self.j[4]-self.j[7], self.j[5]]
                for n in range(len(self.j[13])):
                    lab2_multi.append([self.j[13][n][0]-self.j[6], self.j[13][n][1]-self.j[7], self.j[13][n][2]-self.j[6], self.j[13][n][3]-self.j[7], self.j[13][n][4]])
                
                img3 = img3[self.k[7]:self.k[9], self.k[6]:self.k[8]]
                lab3_whole = [self.k[1]-self.k[6], self.k[2]-self.k[7], self.k[3]-self.k[6], self.k[4]-self.k[7], self.k[5]]
                for n in range(len(self.k[13])):
                    lab3_multi.append([self.k[13][n][0]-self.k[6], self.k[13][n][1]-self.k[7], self.k[13][n][2]-self.k[6], self.k[13][n][3]-self.k[7], self.k[13][n][4]])
                
                img4 = img4[self.v[7]:self.v[9], self.v[6]:self.v[8]]
                lab4_whole = [self.v[1]-self.v[6], self.v[2]-self.v[7], self.v[3]-self.v[6], self.v[4]-self.v[7], self.v[5]]
                for n in range(len(self.v[13])):
                    lab4_multi.append([self.v[13][n][0]-self.v[6], self.v[13][n][1]-self.v[7], self.v[13][n][2]-self.v[6], self.v[13][n][3]-self.v[7], self.v[13][n][4]])
                
                # 调用合成函数合成图片
                four_to_one_less_config1_synthetise(img1, img2, img3, img4, lab1_whole, lab2_whole, lab3_whole, lab4_whole, lab1_multi, lab2_multi, lab3_multi, lab4_multi, 
...
def four_to_one_less_config1_synthetise(img1, img2, img3, img4, lab1_whole, lab2_whole, lab3_whole, lab4_whole, 
                                        lab1_multi, lab2_multi, lab3_multi, lab4_multi,
    ...
    # 依据左上角需要的宽高比例来确定是否旋转左上图块 ---------------------------------        这里是依据图块坐标判断是否需要旋转图块, 顺带旋转目标
    img1, lab1_whole, lab1_multi = check_objectorient(new_w * ratio_width_top, new_h * ratio_height, img1, lab1_whole, lab1_multi)
    
    # 检查目标宽高是否大于空位宽高,并缩放大于空位宽高的目标                                    这里需要用大目标整体坐标, 但是只取得了多目标坐标, 该步需要放到最后
    img1, lab1_whole, lab1_multi = check_objectsize(new_w * ratio_width_top, new_h * ratio_height, img1, lab1_whole, lab1_multi)
    
    # 单个目标的图块切割前确定获取的图块的目标小于32×32,并且目标宽高小于空位宽高                  这里同样是依据大目标坐标判断是否需要缩放大目标, 大概的修改点就在这里, 确保多个目标均小于阈值
    if lab1_whole[0] == lab1_multi[0] and lab1_whole[1] == lab1_multi[1] and lab1_whole[2] == lab1_multi[2] and lab1_whole[3] == lab1_multi[3]:
        img1, lab1_whole, lab1_multi = shrink_object(img1, lab1_whole, lab1_multi, scope, domain_list = None)
    else: # 多目标列表分析目标群模式, 然后再根据模式选择方案进行缩放处理
        img1, lab1_whole, lab1_multi = shrink_object(img1, lab1_whole, lab1_multi, scope, domain_obj = analyze_pattern(lab1_multi))
        
    # 当图块与空位形状相似但宽高不同需要缩放时,进行切割,只填充相应方向,缩小目标畸变与四个图块中间的灰色区域    这里是依据图块宽高与大目标整体坐标判断切割与填充
    if (new_w * ratio_width_top / img1.shape[1]) == 1 and (new_h * ratio_height / img1.shape[0]) == 1:
        pass
    else:
        img1, lab1_whole, lab1_multi = correct_distortion_patch1(new_w * ratio_width_top, new_h * ratio_height, img1, lab1_whole, lab1_multi, count) # ) # 
    
    # 依据右上角需要的宽高比例来确定是否旋转右上图块 ---------------------------------
    img2, lab2_whole, lab2_multi = check_objectorient(new_w * (1-ratio_width_top), new_h * ratio_height, img2, lab2_whole, lab2_multi)
    
    # 检查目标宽高是否大于空位宽高,并缩放大于空位宽高的目标
    img2, lab2_whole, lab2_multi = check_objectsize(new_w * (1-ratio_width_top), new_h * ratio_height, img2, lab2_whole, lab2_multi)
    
    # 单个目标的图块切割前确定获取的图块的目标小于32×32,并且目标宽高小于空位宽高
    if lab2_whole[0] == lab2_multi[0] and lab2_whole[1] == lab2_multi[1] and lab2_whole[2] == lab2_multi[2] and lab2_whole[3] == lab2_multi[3]:
        img2, lab2_whole, lab2_multi = shrink_object(img2, lab2_whole, lab2_multi, scope, domain_list = None)
    else: # 多目标列表分析目标群模式, 然后再根据模式选择方案进行缩放处理
        img2, lab2_whole, lab2_multi = shrink_object(img2, lab2_whole, lab2_multi, scope, domain_obj = analyze_pattern(lab2_multi))
    
    # 进行切割然后填充以避免后续处理时的畸变
    if (new_w * (1-ratio_width_top) / img2.shape[1]) == 1 and (new_h * ratio_height / img2.shape[0]) == 1:
        pass
    else:
        img2, lab2_whole, lab2_multi = correct_distortion_patch2(new_w * (1-ratio_width_top), new_h * ratio_height, img2, lab2_whole, lab2_multi, count) # ) # 
    ...
    lab1_whole, lab1_multi = multiply_ratio_to_list(lab1_whole, lab1_multi, ratios_1)
    lab2_whole, lab2_multi = multiply_ratio_to_list(lab2_whole, lab2_multi, ratios_2)
    ...
    lab1_whole, lab1_multi = add_offset_to_list(lab1_whole, lab1_multi, offsets[0])
    lab2_whole, lab2_multi = add_offset_to_list(lab2_whole, lab2_multi, offsets[1])
    
    # 依据左下角需要的宽高比例来确定是否旋转左下图块 ---------------------------------
    img3, lab3_whole, lab3_multi = check_objectorient(new_w * ratio_width_bot, new_h * (1-ratio_height), img3, lab3_whole, lab3_multi)
    
    # 检查目标宽高是否大于空位宽高,并缩放大于空位宽高的目标
    img3, lab3_whole, lab3_multi = check_objectsize(new_w * ratio_width_bot, new_h * (1-ratio_height), img3, lab3_whole, lab3_multi)
    
    # 单个目标的图块切割前确定获取的图块的目标小于32×32,并且目标宽高小于空位宽高
    if lab3_whole[0] == lab3_multi[0] and lab3_whole[1] == lab3_multi[1] and lab3_whole[2] == lab3_multi[2] and lab3_whole[3] == lab3_multi[3]:
        img3, lab3_whole, lab3_multi = shrink_object(img3, lab3_whole, lab3_multi, scope, domain_list = None)
    else: # 多目标列表分析目标群模式, 然后再根据模式选择方案进行缩放处理
        img3, lab3_whole, lab3_multi = shrink_object(img3, lab3_whole, lab3_multi, scope, domain_obj = analyze_pattern(lab3_multi))
    ...
    if (new_w * ratio_width_bot / img3.shape[1]) == 1 and (new_h * (1-ratio_height) / img3.shape[0]) == 1:
        ...
        img3, lab3_whole, lab3_multi = correct_distortion_patch3(new_w * ratio_width_bot, new_h * (1-ratio_height), img3, lab3_whole, lab3_multi, count) # ) # 
    
    # 依据右下角需要的宽高比例来确定是否旋转右下图块 ---------------------------------
    img4, lab4_whole, lab4_multi = check_objectorient(new_w * (1-ratio_width_bot), new_h * (1-ratio_height), img4, lab4_whole, lab4_multi)
    
    # 检查目标宽高是否大于空位宽高,并缩放大于空位宽高的目标
    img4, lab4_whole, lab4_multi = check_objectsize(new_w * (1-ratio_width_bot), new_h * (1-ratio_height), img4, lab4_whole, lab4_multi)
    
    # 单个目标的图块切割前确定获取的图块的目标小于32×32,并且目标宽高小于空位宽高
    if lab4_whole[0] == lab4_multi[0] and lab4_whole[1] == lab4_multi[1] and lab4_whole[2] == lab4_multi[2] and lab4_whole[3] == lab4_multi[3]:
        img4, lab1_whole, lab4_multi = shrink_object(img4, lab4_whole, lab4_multi, scope, domain_list = None)
    else: # 多目标列表分析目标群模式, 然后再根据模式选择方案进行缩放处理
        img4, lab1_whole, lab4_multi = shrink_object(img4, lab4_whole, lab4_multi, scope, domain_obj = analyze_pattern(lab4_multi))
    
    # 进行切割然后填充以避免后续处理时的畸变
    if (new_w * (1-ratio_width_bot) / img4.shape[1]) == 1 and (new_h * (1-ratio_height) / img4.shape[0]) == 1:
        pass
    else:
        img4, lab4_whole, lab4_multi = correct_distortion_patch4(new_w * (1-ratio_width_bot), new_h * (1-ratio_height), img4, lab4_whole, lab4_multi, count) # ) # 
    ...
    lab3_whole, lab3_multi = multiply_ratio_to_list(lab3_whole, lab3_multi, ratios_3)
    lab4_whole, lab4_multi = multiply_ratio_to_list(lab4_whole, lab4_multi, ratios_4)
    ...
    lab3_whole, lab3_multi = add_offset_to_list(lab3_whole, lab3_multi, offsets[0])
    lab4_whole, lab4_multi = add_offset_to_list(lab4_whole, lab4_multi, offsets[1])
    ...
    # 为坐标加上填充量
    lab1_whole, lab1_multi = add_pad_to_list(lab1_whole, lab1_multi, pad_top, pad_left)
    lab2_whole, lab2_multi = add_pad_to_list(lab2_whole, lab2_multi, pad_top, pad_left)
    lab3_whole, lab3_multi = add_pad_to_list(lab3_whole, lab3_multi, pad_top, pad_left)
    lab4_whole, lab4_multi = add_pad_to_list(lab4_whole, lab4_multi, pad_top, pad_left)
    
    cv2.imwrite(save_dir + str(count) + '.jpg', syn_img3) # cv2.imwrite reqire [h, w, c]
    
    temp_list1 = [lab1_multi[0]]
    for i in range(1, len(lab1_multi)):
        temp_list1 = torch.cat((torch.from_numpy(np.array(temp_list1)),
                                torch.from_numpy(np.array([lab1_multi[i]]))), 0)
    
    temp_list2 = [lab2_multi[0]]
    for i in range(1, len(lab2_multi)):
        temp_list2 = torch.cat((torch.from_numpy(np.array(temp_list2)),
                                torch.from_numpy(np.array([lab2_multi[i]]))), 0)
    
    temp_list3 = [lab3_multi[0]]
    for i in range(1, len(lab3_multi)):
        temp_list3 = torch.cat((torch.from_numpy(np.array(temp_list3)),
                                torch.from_numpy(np.array([lab3_multi[i]]))), 0)
    
    temp_list4 = [lab4_multi[0]]
    for i in range(1, len(lab4_multi)):
        temp_list4 = torch.cat((torch.from_numpy(np.array(temp_list4)),
                                torch.from_numpy(np.array([lab4_multi[i]]))), 0)
    
    label_tensor = torch.cat((torch.from_numpy(np.array(temp_list1)), 
                              torch.from_numpy(np.array(temp_list2)), 
                              torch.from_numpy(np.array(temp_list3)), 
                              torch.from_numpy(np.array(temp_list4))), 0)
