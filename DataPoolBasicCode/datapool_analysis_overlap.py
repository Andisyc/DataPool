...
@author: Pilot Crysi, Overlap Object Analyze of YoloXRe-DST-DataPool
"""
...
# 当输入完全不相交的坐标时就会报错, 因为target_bool会拼接到None
...
def situation_3(target, list_target, res, height, width): # 输入该目标的三个列表, 返回单个目标的扩展信息
    ...
    if len(list_target[2]) == 1:
        if list_target[2][0][0] <= target[0] < target[2] <= list_target[2][0][2] and  \
            list_target[2][0][1] <= target[1] < target[3] <= list_target[2][0][3]:
            if target[0] >= 10:
                left = 10
            else:
                left = target[0]
...
def estimate_point(tensor1, tensor2): # 判断tensor1的顶点是否在tensor2中
...
def overlap(tensor1, tensor2): # torchvision.ops.box_iou(torch.from_numpy(res[i][:4]).unsqueeze(0), torch.from_numpy(res[j][:4]).unsqueeze(0)) > 0:
...
def compute_ratio(res, cutinfo): # 计算图块/目标比例方便检索
    ratio = []
...
def write_into_csv(res, cutinfo, ratio, csv_path): # 写入csv文件,图片名 目标坐标(xyxycls) 裁剪坐标(xyxy) 目标面积 裁剪面积 图块/目标比例
    for i in range(res.shape[0]):
        for j in range(len(cutinfo[i])):
...
if __name__ == "__main__":
    ...
    # 测试图片与标签路径
    img_path = 'D:/AICV-DSTRethink/Code-DataPoolTest&Results/singleimg/000138.jpg'
    xml_path = 'D:/AICV-DSTRethink/Code-DataPoolTest&Results/singlelab/000138.xml'
    csv_path = 'D:/AICV-DSTRethink/Code-DataPoolTest&Results/VOC_0712trainval_ObjectInfo.csv'
    vis_path = 'D:/AICV-DSTRethink/Code-DataPoolTest&Results/singleres/'
    ...
    # ratio = compute_ratio(res, cutinfo) # 计算图块/目标比例方便检索
    ...
    for i in range(len(cutinfo)):
        ...
        cv2.imwrite(vis_path + str(os.path.basename(img_path)) + '_' + str(i) + '_' + str(j) + '.jpg', img_temp)
    
    # write_into_csv(res, cutinfo, ratio, csv_path) # 写入csv文件,图片名 目标坐标(xyxycls) 裁剪坐标(xyxy) 目标面积 裁剪面积 图块/目标比例