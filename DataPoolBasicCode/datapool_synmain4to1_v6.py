...
from datapool_synthauxiliary_v6 import (xml_label_generate, shrink_object_single, shrink_object_cluster, check_objectsize, check_objectorient, 
                                        ...
                                        calculate_cluster_meanarea, filp_label_90, decrypt_coor, normlize_index, elimate_singleobj, visualize_patch)

img_file = 'F:/VOCtrainval_images/' # D:/AICV-DSTRethink/DataPool/vocdataset/images/
save_dir = 'D:\AICV-DSTRethink\Code-DataPoolTest&Results\singleres/'
...
class four_to_one_less_config1_search:
    ...
    def do_search(self, save_dir, scope, extre_small):
        # 检查文件夹是否存在,不存在则创建文件夹
        ...
        for i in range(len(list_target)):
            ...
            # proportion_1_low即空位面积/1024, 这是图块/目标比例的最小值, 低于这个最小值则说明目标过大, 图块缩放后目标仍然大于32×32
            # proportion_1_low这个值增大即目标增加, 导致缩放后目标仍然大于32×32, 减小即目标减小, 缩放后目标会小于32×32
            # search_area_1_low即空位面积, 这里要求图块面积大于空位面积, 即只采用大于空位面积的图块再对其进行缩小, 不对小图块放大
            # 还是要设置限制措施, 否则会因为后续缩放出现超大灰色区域, 可以改成目标的mean大于proportion_1_low即可
            # 这里还需要加上测试目标在缩放后是否会变得过小的功能, 使用这个功能来筛选图块比当前使用图块与目标的面积比例更好
            
            # 判断该目标是否满足search_area_1的条件
            if proportion_1_high == 'infinite':
                if len(list2) > 13:
                    if list2[11] / calculate_cluster_meanarea(decrypt_coor(list2)) > proportion_1_low and list2[11] > search_area_1_low:
            ...
            if proportion_2_high == 'infinite':
                if len(list2) > 13:
                    if list2[11] / calculate_cluster_meanarea(decrypt_coor(list2)) > proportion_2_low and list2[11] > search_area_2_low:
            ...
            if proportion_3_high == 'infinite':
                if len(list2) > 13:
                    if list2[11] / calculate_cluster_meanarea(decrypt_coor(list2)) > proportion_3_low and list2[11] > search_area_3_low:
            ...
            if proportion_4_high == 'infinite':
                if len(list2) > 13:
                    if list2[11] / calculate_cluster_meanarea(decrypt_coor(list2)) > proportion_4_low and list2[11] > search_area_4_low:
        ...
        # 排除图块列表中只具有单个目标的图块, 但列表中只有单个目标图块则不修改列表
        self.potential_area_1 = elimate_singleobj(self.potential_area_1)
        self.potential_area_2 = elimate_singleobj(self.potential_area_2)
        self.potential_area_3 = elimate_singleobj(self.potential_area_3)
        self.potential_area_4 = elimate_singleobj(self.potential_area_4)
        ...
        if len(self.potential_area_1) != 0 and len(self.potential_area_2) != 0 and len(self.potential_area_3) != 0 and len(self.potential_area_4) != 0:
            ...
            while(len(self.potential_area_1) != 0 or len(self.potential_area_2) != 0 or len(self.potential_area_3) != 0 or len(self.potential_area_4) != 0):
                ...
                img1 = img1[self.i[7]:self.i[9], self.i[6]:self.i[8]] # [ymin: ymax, xmin: xmax]
                ...
                """
                # lab这四个坐标会出现负数坐标的原因是目标本该同时存在于切割区域内, 但有些
                # 目标存在于切割区域外, 在计算新坐标时就出现了负数(调试代码不可删除)
                for m in range(len(lab1)):
                    assert lab1[m] >= 0, f"{lab1}, {self.i}"
                for m in range(len(lab2)):
                    assert lab2[m] >= 0, f"{lab2}, {self.j}"
                for m in range(len(lab3)):
                    assert lab3[m] >= 0, f"{lab3}, {self.k}"
                for m in range(len(lab4)):
                    assert lab4[m] >= 0, f"{lab4}, {self.v}"
                
                # 可视化图块确定的确标签读取正确(调试代码不可删除)
                print(count, "patch 1: ", len(cluster_i), len(self.i))
                visualize_patch(lab1, cluster_i, img1, save_dir, count, 1, self.i[0])
                print(count, "patch 2: ", len(cluster_j), self.j[0])
                visualize_patch(lab2, cluster_j, img2, save_dir, count, 2, self.j[0])
                print(count, "patch 3: ", len(cluster_k), self.k[0])
                visualize_patch(lab3, cluster_k, img3, save_dir, count, 3, self.k[0])
                print(count, "patch 4: ", len(cluster_v), self.v[0])
                visualize_patch(lab4, cluster_v, img4, save_dir, count, 4, self.v[0])
                print("-" * 20)
                """
                ...
                four_to_one_less_config1_synthetise(img1, img2, img3, img4, lab1, lab2, lab3, lab4,
                                                    ...
                                                    count, self.height, self.width, save_dir, scope, extre_small)
...
def four_to_one_less_config1_synthetise(img1, img2, img3, img4, lab1, lab2, lab3, lab4, 
                                        ...
                                        count, height, width, save_dir, scope, extre_small):
    ...
    """
    # 可视化代码
    temp_img = copy.deepcopy(img1)
    # temp_img = np.ascontiguousarray(np.transpose(temp_img, (1, 2, 0)))
    
    # 可视化拼贴图片的标签是否与目标匹配,结果正确
    label_tensor = torch.from_numpy(np.array([lab1]))
    ...
    for i in range(len(label_tensor)):
        if type(label_tensor[i]) != list:
            label_tensor[i] = label_tensor[i].tolist()
    for i in range(len(cluster_object)):
        if type(cluster_object[i]) != list:
            cluster_object[i] = cluster_object[i].tolist()
    
    # 使用占位符替换目标簇中小于极限面积的目标
    for i in range(len(cluster_object)):
        if (cluster_object[i][2] - cluster_object[i][0]) * (cluster_object[i][3] - cluster_object[i][1]) < extre_small:
            cluster_object[i] = 'occ'
    
    # 剔除占位符occ, 此时cluster_object为list
    while 'occ' in cluster_object:
        cluster_object.remove('occ')
    ...
    label_tensor = torch.cat((torch.from_numpy(np.array(label_tensor)), torch.from_numpy(np.array(cluster_object))), 0)
