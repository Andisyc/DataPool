...
def get_aug_params(value, center=0):
    if isinstance(value, float):
        return random.uniform(center - value, center + value) # random.uniform(x, y)在x与y范围内随机生成一个实数
    elif len(value) == 2:
...
def get_affine_matrix(target_size, degrees=10, translate=0.1, scales=0.1, shear=10):
    ...
    # Rotation and Scale
    angle = get_aug_params(degrees) # 随机生成旋转角度
    scale = get_aug_params(scales, center=1.0) # 随机生成缩放尺度

    if scale <= 0.0:
        raise ValueError("Argument scale should be positive")

    R = cv2.getRotationMatrix2D(angle=angle, center=(0, 0), scale=scale)
    # cv2.getRotationMatrix2D生成旋转矩阵, 以左上点为原点旋转一定的角度再缩放
...
def random_affine(img, targets=(), target_size=(640, 640), degrees=10, translate=0.1, scales=0.1, shear=10):
    ...
    if len(targets) > 0:
        targets = apply_affine_to_bboxes(targets, target_size, M, scale) # 标签, 设定尺寸
