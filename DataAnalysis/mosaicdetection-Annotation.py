...
def get_mosaic_coordinate(mosaic_image, mosaic_index, xc, yc, w, h, input_h, input_w):
    # TODO update doc 合成图, 当前图片, 中心点坐标, 当前图片宽高, 设定宽高
    # index0 to top left part of image
    if mosaic_index == 0: # 右下方对齐中心点, 左上方超出范围则舍弃
        x1, y1, x2, y2 = max(xc - w, 0), max(yc - h, 0), xc, yc # 合成图粘贴区域坐标
        small_coord = w - (x2 - x1), h - (y2 - y1), w, h
        # 当前图片宽/高大于合成图左上点到中心点时, small_coord为当前图片的切割坐标
        # 当前图片宽/高小于合成图左上点到中心点时, small_coord为当前图片整张图片
...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            # yc, xc = s, s  # mosaic center x, y 随机生成中心点
            yc = int(random.uniform(0.5 * input_h, 1.5 * input_h))
            xc = int(random.uniform(0.5 * input_w, 1.5 * input_w))

            # 3 additional image indices 从数据集中随机抽取3张图片
            indices = [idx] + [random.randint(0, len(self._dataset) - 1) for _ in range(3)]

            for i_mosaic, index in enumerate(indices):
                img, _labels, _, img_id = self._dataset.pull_item(index) # 读取图片
                h0, w0 = img.shape[:2]  # orig hw 取得宽高
                scale = min(1. * input_h / h0, 1. * input_w / w0) # 判断是否存在某边小于设定尺寸
                img = cv2.resize(img, (int(w0 * scale), int(h0 * scale)), interpolation=cv2.INTER_LINEAR)
                # 将短于设定尺寸的边缩放至设定尺寸, 就算另一边会大于设定尺寸也照样放大
                
                # generate output mosaic image
                (h, w, c) = img.shape[:3]
                if i_mosaic == 0: # 合成图宽高为设定尺寸的2倍
                ...
                padw, padh = l_x1 - s_x1, l_y1 - s_y1 # 当前图片大于空位时pad小于0, 当前图片小于空位时pad大于0

                labels = _labels.copy()
                # Normalized xywh to pixel xyxy format 将归一化的xywh缩放至xyxy 这里的意思大概是到这一步坐标已经是还原的xyxy格式
                if _labels.size > 0: # 这里不知道为啥torch.size可以与int比较
                    labels[:, 0] = scale * _labels[:, 0] + padw # 相当于目标宽高不变, 但是将目标位置坐标整体迁移
                    labels[:, 1] = scale * _labels[:, 1] + padh # 当前图片大于空位时, 可能坐标会出现负数
                    labels[:, 2] = scale * _labels[:, 2] + padw # 但是后续会将坐标值限制在0-input值范围内
                    labels[:, 3] = scale * _labels[:, 3] + padh
                mosaic_labels.append(labels)

            if len(mosaic_labels):
                mosaic_labels = np.concatenate(mosaic_labels, 0)
                np.clip(mosaic_labels[:, 0], 0, 2 * input_w, out=mosaic_labels[:, 0]) # 将参数1至于参数2和参数3之间
                np.clip(mosaic_labels[:, 1], 0, 2 * input_h, out=mosaic_labels[:, 1]) # 小于参数2的值则等于参数2
                np.clip(mosaic_labels[:, 2], 0, 2 * input_w, out=mosaic_labels[:, 2]) # 大于参数3的值则等于参数3
            ...
            # img_info and img_id are not used for training. They are also hard to be specified on a mosaic image.
    ...
    def mixup(self, origin_img, origin_labels, input_dim): # 明明是随机抖动为啥TM写Mixup???
