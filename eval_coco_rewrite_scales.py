...
line 1965: Weight path
line 1599: Dataset path
line 1585: Class number
"""
import io
import os
import cv2
...
COCO_CLASSES = ("aeroplane", "bicycle", "bird", "boat", "bottle",
...
def parse_rec(filename):
    ...
    """
    path = "/mnt/yoloxrecuda/VOCdevkit/VOC2007/Annotations"
...
def per_class_AR_table(coco_eval, class_names=COCO_CLASSES, headers=["class", "AR"], colums=6):
    ...
    table = tabulate(row_pair, tablefmt="pipe", floatfmt=".3f", headers=table_headers, numalign="left")
    return table
...
class COCOEvaluator:
    ...
    """
    def __init__(self, dataloader, img_size: int, confthre: float, nmsthre: float, num_classes: int,
                 testdev: bool = False, per_class_AP: bool = False, per_class_AR: bool = False):
        """
        Args:
    ...
    def convert_to_coco_format(self, outputs, info_imgs, ids):
        ...
        for (output, img_h, img_w, img_id) in zip(outputs, info_imgs[0], info_imgs[1], ids):
            ...
            for ind in range(bboxes.shape[0]):
                ...
                pred_data = {"image_id": int(img_id),
                             "category_id": label,
                             "bbox": bboxes[ind].numpy().tolist(),
                             "score": scores[ind].numpy().item(),
                             "segmentation": [],}  # COCO json format
...
class Exp():
    def __init__(self, data_dir, val_ann):
        super().__init__()
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False):
        valdataset = COCODataset(data_dir = self.data_dir,
...
def get_exp_by_file(exp_file, data_dir, val_ann):
    try:
        # sys.path.append(os.path.dirname(exp_file))
        # current_exp = importlib.import_module(os.path.basename(exp_file).split(".")[0])
        # exp = current_exp.Exp()
        exp = Exp(data_dir, val_ann)
    except Exception:
...
def get_exp(exp_file, exp_name, data_dir, val_ann):
    """
    ...
    if exp_file is not None:
        return get_exp_by_file(exp_file, data_dir, val_ann)
    else:
...
def make_parser():
    ...
    parser.add_argument("-c", "--ckpt", default="D:/AICV-DSTRethink/Experiments_FailedRun/YoloXReMosaicALD_0.2_20_5/BestAP76.23_97epoch_16bs_100epoch/best_ckpt.pth", type=str, help="ckpt for eval")
    parser.add_argument("-img", "--images", default="D:/AICV-DSTRethink/Datasets_VOC07test/VOCdevkit/VOC2007/JPEGImages/" , type=str, help="normal form datasets")
    parser.add_argument("-lab", "--labels", default="D:/AICV-DSTRethink/val.json" , type=str, help="json form labels")
...
if __name__ == "__main__":
    ...
    exp = get_exp(args.exp_file, args.name, args.images, args.labels)
    exp.merge(args.opts)
    ...
    launch(main, num_gpu, args.num_machines, args.machine_rank, backend=args.dist_backend, dist_url=dist_url, args=(exp, args, num_gpu))