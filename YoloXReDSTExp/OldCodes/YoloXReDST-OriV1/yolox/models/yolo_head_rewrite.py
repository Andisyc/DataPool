# -*- coding: utf-8 -*-
"""
Created on Thu Mar  3 16:50:16 2022
...
class YOLOXHead(nn.Module):
    ...
    def get_losses(self, imgs, x_shifts, y_shifts, expanded_strides, labels, outputs, origin_preds, dtype):
            ...
            if num_gt == 0:
                ...
                try:
                    ...
                    (gt_matched_classes,fg_mask,pred_ious_this_matching,matched_gt_inds,num_fg_img,) = self.get_assignments(  # noqa
                        flag, batch_idx, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, expanded_strides,
        ...
        """
        print("small_index: ", small_index, small_index.shape)
        print("medium_index: ",medium_index, medium_index.shape)
        print("large_index: ", large_index, large_index.shape)
        """
        
        # 计算本轮次各个损失值
        num_fg = max(num_fg, 1)
        lossiou = self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)
        loss_iou = lossiou.sum() / num_fg
        # loss_iou = (self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)).sum() / num_fg # 这两者shape相等
        ...
        """
        print("iouloss: ", lossiou, lossiou.shape)
        print("bbox_preds.view(-1, 4)[fg_masks]: ", bbox_preds.view(-1, 4)[fg_masks].shape)
        print("reg_targets: ", reg_targets.shape) # 这三者shape[0]相等,即Pred&GT中能匹配的图片数量
        """
        
        # 计算损失值中小目标比例(注意: 标量的shape为空元组)
        if lossiou.shape[0] == 0: # lossiou为空张量时
            ratio_small = 0.0
        elif lossiou.shape[0] == 1: # lossiou只有1个值时
            if small_index[0] == True:
                ratio_small = 1.0
            else:
                ratio_small = 0.0
        else: # lossiou为多值向量时
            ratio_small = lossiou[small_index].sum() / lossiou.sum()
            ratio_medium = lossiou[medium_index].sum() / lossiou.sum()
            ratio_large = lossiou[large_index].sum() / lossiou.sum()
        
        # print("small_index: ", small_index)
        # print("ratio_small: ", ratio_small)
        print("loss_small: ", lossiou[small_index].sum())
        print("loss_medium: ", lossiou[medium_index].sum())
        print("loss_large: ", lossiou[large_index].sum())
        print("loss_sum: ", lossiou.sum())
        
        ratio_scale = [ratio_small, ratio_medium, ratio_large]
