...
import sys
# sys.path.append(r'D:/AICV-YoloXReDST-New')
...
class Trainer:
    ...
    def train(self):
        ...
        self.train_loader_ir = self.exp.get_data_loader_ir(batch_size=self.args.batch_size,
                                                        is_distributed=self.is_distributed,
                                                        no_aug=self.no_aug,
                                                        cache_img=self.args.cache)
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                ...
                if (self.iter + 1) % self.exp.print_interval == 0:
                    ...
                    logger.info("{}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), time_str, loss_str, self.meter["lr"].latest) + \
                               (", size: {:d}, {}".format(self.input_size[0], eta_str)) + (", re_iter: {}, ir_iter: {}".format(self.iter_normal, self.iter_unnorm)) + \
