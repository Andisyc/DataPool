...
import sys
sys.path.append(r'D:/AICV-YoloXReDST-Orig')
...
class Trainer:
    ...
    def train(self):
        ...
        self.prefetcher_s = DataPrefetcher(self.exp.get_data_loader_ir(batch_size=self.args.batch_size * 4,
                                                                             is_distributed=self.is_distributed,
                                                                             no_aug=self.no_aug,
                                                                             cache_img=self.args.cache))
        self.prefetcher_m = DataPrefetcher(self.exp.get_data_loader_ir(batch_size=self.args.batch_size * 4,
                                                                              is_distributed=self.is_distributed,
                                                                              no_aug=self.no_aug,
                                                                              cache_img=self.args.cache))
        self.prefetcher_l = DataPrefetcher(self.exp.get_data_loader_ir(batch_size=self.args.batch_size * 4,
                                                                             is_distributed=self.is_distributed,
                                                                             no_aug=self.no_aug,
                                                                             cache_img=self.args.cache))
        ...
        # 用数据提取器包装数据加载器
        self.prefetcher_re = DataPrefetcher(self.train_loader)
        ...
        # 初始化损失值累加器和布尔变量
        loss_log, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large = None, 0.0, 0.0, 0.0
        self.batch_flag, self.is_regular, self.batch_counter, self.iter_normal = [0], True, 0, 0
        self.iter_small, self.iter_medium, self.iter_large = 0, 0, 0
        
        # 强行设定最大迭代次数
        self.max_iter = self.dataset_len * 300
        ...
        for self.iter in range(self.max_iter):
            ...
            if (self.epoch + 1) % 20 == 0:
                # 将损失值组装成键值对
                self.sorted_values = self.get_aug_scale()
                loss_log.append(self.loss_log)
                
                # 取得最小值并使得最小值为0
                if self.sorted_values[0] == 'small':
                ...
                if self.sorted_values[0] == 'medium':
            ...
            if self.is_regular == True:
                if self.re_batch_counter % 10 == 0:        # 每3个正常batch检查一次损失值之间的差异
                    self.batch_flag = self.get_aug_scale() # 判断三尺度累加值中最小值, 选定需要增强的尺度
                    if self.batch_flag[0] != 0:
                        self.is_regular, self.re_batch_counter, self.temp_iter = False, 0, self.iter
                        logger.info("------- begin" + self.batch_flag[0] + "batch -------")
            else:
                if self.batch_flag[1] <= self.batch_flag[0] or self.batch_flag[2] <= self.batch_flag[0]:
                    self.batch_flag[0], self.is_regular = 0, True
                    logger.info("--- ending with " + str(self.iter - self.temp_iter) + " extra batch ---")
            
            # 判断进入哪种模式
            if self.batch_flag[0] == 0:                     # train_one_iter regular
                self.prefetcher = self.prefetcher_re
                self.batch_counter = self.batch_counter + 1 # regular batch counter
                self.iter_normal = self.iter_normal + 1
            elif self.batch_flag[0] == 'small':             # train_one_iter small
                self.prefetcher = self.prefetcher_s
                self.iter_small = self.iter_small + 1
            elif self.batch_flag[0] == 'medium':            # train_one_iter medium
                self.prefetcher = self.prefetcher_m
                self.iter_medium = self.iter_medium + 1
            else:                                           # train_one_iter large
                self.prefetcher = self.prefetcher_l
                self.iter_large = self.iter_large + 1
            
            # iter_start_time = time.time()
                
            inps, targets = self.prefetcher.next() # [2,3,640,640]&[2,120,5]
            ...
            if (self.iter + 1) % self.exp.print_interval == 0:
                ...
                logger.info("{}, mem: {:.0f}Mb, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), loss_str, self.meter["lr"].latest) + \
                                (", re_batch: {}, s_batch: {}, m_batch: {}, l_batch: {}, s_loss: {}, m_loss: {}, l_loss: {}".format(self.iter_normal, 
                                self.iter_small, self.iter_medium, self.iter_large, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large)))
            ...
            if (self.iter + 1 // self.dataset_len + 1) % self.exp.eval_interval == 0:
                all_reduce_norm(self.model)
                self.evaluate_and_save_model()
        
            
            # save weight per epoch
            if (self.iter_normal + 1) % self.dataset_len == 0:
                self.save_ckpt(ckpt_name="latest")
            
            # evaluate and save weight
            if (self.iter_normal + 1) % (self.exp.eval_interval * self.dataset_len) == 0:
                all_reduce_norm(self.model)
                self.evaluate_and_save_model()
    ...
    def get_aug_scale(self):
        ...
        return sorted(self.loss_log, key=self.loss_log.get)[0]
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                self.tblogger.add_scalar("val/COCOAP50", ap50, self.iter + 1 // self.dataset_len + 1) # self.epoch
                self.tblogger.add_scalar("val/COCOAP50_95", ap50_95, self.iter + 1 // self.dataset_len + 1) # self.epoch
            if self.args.logger == "wandb":
                self.wandb_logger.log_metrics({"val/COCOAP50": ap50, "val/COCOAP50_95": ap50_95, "epoch": self.iter+1//self.dataset_len+1})
            logger.info("\n" + summary)
        synchronize()

        self.save_ckpt("last_epoch", update_best_ckpt)
        if self.save_history_ckpt:
            self.eopch_num = (self.iter + 1) // self.dataset_len
            self.save_ckpt(f"epoch_{self.eopch_num}") # self.epoch
