...
import sys
sys.path.append(r'/mnt/yoloxredstadp')
...
class Trainer:
    ...
    def train(self):
        ...
        self.prefetcher_s = DataPrefetcher(self.exp.get_data_loader_s(batch_size=self.args.batch_size,
                                                                      is_distributed=self.is_distributed,
                                                                      no_aug=self.no_aug,
                                                                      cache_img=self.args.cache))
        """
        self.prefetcher_m = DataPrefetcher(self.exp.get_data_loader_s(batch_size=self.args.batch_size * 4,
                                                                      is_distributed=self.is_distributed,
                                                                      no_aug=self.no_aug,
                                                                      cache_img=self.args.cache))
        self.prefetcher_l = DataPrefetcher(self.exp.get_data_loader_s(batch_size=self.args.batch_size * 4,
                                                                      is_distributed=self.is_distributed,
                                                                      no_aug=self.no_aug,
                                                                      cache_img=self.args.cache))
        """
        ...
        # 初始化损失值累加器和布尔变量, re_batch_counter正常批次计数器, iter_unnorm非正常批次总数
        self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large = 0.0, 0.0, 0.0 # loss_log = []
        self.batch_flag, self.is_regular, self.re_batch_counter, self.iter_normal = [0], True, 0, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter):
                ...
                if self.is_regular == True:
                    if self.re_batch_counter % 10 == 0: # 10个正常批次检查1次损失值差异
                        self.batch_flag = self.get_aug_scale()
                        zero_values, first_values, second_values = self.get_loss_values()
                        if int(first_values) * 0.1 <= int(zero_values) or int(second_values) * 0.1 <= int(zero_values):
                            self.batch_flag = [0]
                        else:
                            pass
                ...
                if self.batch_flag[0] == 0:                           # train_one_iter regular
                    self.prefetcher = self.prefetcher_re
                    self.re_batch_counter = self.re_batch_counter + 1 # regular batch counter
                ...
                if (self.iter + 1) % self.exp.print_interval == 0:
                    ...
                    logger.info("{}, mem: {:.0f}Mb, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), loss_str, self.meter["lr"].latest) + \
                                (", re_batch: {}, s_batch: {}, s_loss: {}, m_loss: {}, l_loss: {}".format(self.iter_normal, #  m_batch: {}, l_batch: {},
                                self.iter_small, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large))) #  self.iter_medium, self.iter_large,
    ...
    def get_loss_values(self):
        if self.batch_flag[0] == 'small':
            return self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large
        if self.batch_flag[0] == 'medium':
            return self.loss_sum_medium, self.loss_sum_small, self.loss_sum_large
        if self.batch_flag[0] == 'large':
            return self.loss_sum_large, self.loss_sum_medium, self.loss_sum_smalls
