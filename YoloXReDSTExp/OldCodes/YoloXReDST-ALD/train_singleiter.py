# -*- coding: utf-8 -*-
"""
Created on Sat Jul 16 20:13:37 2022
...
from yolox.utils import (MeterBuffer,     # yolox/utils/metric.py
...
class Trainer:
    ...
    def train(self):
        ...
        self.train_loader_small = self.exp.get_data_loader_ir(batch_size=self.args.batch_size * 4,
                                                        is_distributed=self.is_distributed,
                                                        no_aug=self.no_aug,
                                                        cache_img=self.args.cache)
        ...
        self.prefetcher_small = DataPrefetcher(self.train_loader_small)
        
        # max_iter means iters per epoch
        self.dataset_len = len(self.train_loader)
        
        self.lr_scheduler = self.exp.get_lr_scheduler(self.exp.basic_lr_per_img * self.args.batch_size, self.dataset_len)
        ...
        if self.use_model_ema:
            ...
            self.ema_model.updates = self.dataset_len * self.start_epoch
        ...
        loss_log, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large = [], 0.0, 0.0, 0.0
        self.batch_flag, self.is_regular, self.batch_counter, self.iter_normal, self.iter_unnorm = 0, True, 0, 0, 0
        
        # 设定最大迭代次数, 其实在VOC上只要30个轮次就收敛到泛化最佳处叻
        self.max_iter = self.dataset_len * 300 # 是的你没看错, 30个轮次, 1.5W张图片
        
        # train_in_iter 内层for循环 必须将额外轮次也考虑进去才能公平对比各个方法
        for self.iter in range(self.max_iter): # for self.iter in count():
            # 当正常轮次总数达到300时结束迭代
            # if (self.iter_normal + 1) % self.max_iter == 0: break
            
            # 每隔20epoch储存一次损失值,防止损失值爆掉内存
            if (self.iter + 1) % (self.dataset_len * 20) == 0:
                # 将损失值组装成键值对
                loss_dict = {'small': self.loss_sum_small, 'medium': self.loss_sum_medium, 'large': self.loss_sum_large}
                loss_log.append(loss_dict)
                sorted_values = sorted(loss_dict, key=loss_dict.get)
                
                # 取得最小值并使得最小值为0
                if sorted_values[0] == 'small':
                    self.loss_sum_medium = self.loss_sum_medium - self.loss_sum_small
                    self.loss_sum_large = self.loss_sum_large - self.loss_sum_small
                    self.loss_sum_small = 0
                if sorted_values[0] == 'medium':
                    self.loss_sum_small = self.loss_sum_small - self.loss_sum_medium
                    self.loss_sum_large = self.loss_sum_large - self.loss_sum_medium
                    self.loss_sum_medium = 0
                if sorted_values[0] == 'large':
                    self.loss_sum_small = self.loss_sum_small - self.loss_sum_large
                    self.loss_sum_medium = self.loss_sum_medium - self.loss_sum_large
                    self.loss_sum_large = 0
            
            # 首先检查是否在正常批次内
            if self.is_regular == True:
                # 每10个正常batch检查一次损失值之间的差异
                if self.batch_counter % 10 == 0:
                    self.batch_flag = get_aug_scale(self.batch_flag, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large)
                    if self.batch_flag != 0:
                        self.is_regular, self.batch_counter, temp_iter = False, 0, self.iter
                        logger.info("------- begin small batch -------")
            else:
                # 检查损失值差距是否缩小至负数
                if self.loss_sum_medium <= self.loss_sum_small or self.loss_sum_large <= self.loss_sum_small or self.iter - temp_iter == 10:
                    self.batch_flag, self.is_regular = 0, True
                    logger.info("--- ending with " + str(self.iter - temp_iter) + " extra batch ---")
                    if self.rank == 0: # 记录每10个正常轮次存在多少额外轮次
                        if self.args.logger == "tensorboard":
                            self.tblogger.add_scalar("regular/irregular", self.iter - temp_iter, self.iter + 1)
                    del temp_iter
            
            # 判断进入哪种模式
            if self.batch_flag == 0: # train_one_iter regular
            ...
            elif self.batch_flag == 1: # train_one_iter small
                self.prefetcher = self.prefetcher_small
        ...
        for i in range(len(loss_log)):
            logger.info("s:{}, m:{}, l:{}".format(loss_log[i]['small'], loss_log[i]['medium'], loss_log[i]['large']))
        
        # 写入最后一次迭代的损失值
        logger.info("s:{}, m:{}, l:{}".format(self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large))
    ...
    def train_one_iter(self):
        ...
        loss_scale = outputs["loss_scale"]
        self.loss_sum_small = self.loss_sum_small + loss_scale[0]
        self.loss_sum_medium = self.loss_sum_medium + loss_scale[1]
        self.loss_sum_large = self.loss_sum_large + loss_scale[2]
        ...
        lr = self.lr_scheduler.update_lr(self.iter + 1)
        ...
        # if (self.iter_normal + 1) % self.exp.print_interval == 0:
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            progress_str = "iter: {}/{}".format(self.iter + 1, self.max_iter)
            ...
            #             (", re_batch: {}, ir_batch: {}, s_loss: {}, m_loss: {}, l_loss: {}".format(self.iter_normal,
            #             self.iter_unnorm, loss_sum_small, loss_sum_medium, loss_sum_large)))
        ...
        # random resizing
        # if (self.iter + 1) % 10 == 0:
            # self.input_size = self.exp.random_resize(self.train_loader, self.iter + 1 // self.dataset_len, self.rank, self.is_distributed)
            
        # save weight per epoch
        if (self.iter + 1) % self.dataset_len == 0:
            self.save_ckpt(ckpt_name="latest")
            
        # evaluate and save weight
        if (self.iter + 1) % (self.exp.eval_interval * self.dataset_len) == 0:
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                self.tblogger.add_scalar("val/COCOAP50", ap50, self.iter + 1 // self.dataset_len) # self.epoch
                self.tblogger.add_scalar("val/COCOAP50_95", ap50_95, self.iter + 1 // self.dataset_len) # self.epoch
            if self.args.logger == "wandb":
                self.wandb_logger.log_metrics({"val/COCOAP50": ap50, "val/COCOAP50_95": ap50_95, "epoch": self.iter + 1 // self.dataset_len})
            logger.info("\n" + summary)
        synchronize()

        self.save_ckpt("last_epoch", update_best_ckpt)
        if self.save_history_ckpt:
            self.epoch_num = (self.iter + 1) // self.dataset_len
            self.save_ckpt(f"epoch_{self.epoch_num}") # self.epoch

    def save_ckpt(self, ckpt_name, update_best_ckpt=False):
        if self.rank == 0:
            save_model = self.ema_model.ema if self.use_model_ema else self.model
            logger.info("Save weights to {}".format(self.file_name))
            ckpt_state = {"start_epoch": self.iter + 1 // self.dataset_len + 1, # self.epoch
...
def get_aug_scale(batch_flag, loss_sum_small, loss_sum_medium, loss_sum_large):
    if (loss_sum_large - loss_sum_small) > 10:
        batch_flag = 1
    
    if (loss_sum_medium - loss_sum_large) > 10:
        batch_flag = 1
...
if __name__ == "__main__":
    ...
    if world_size > 1: # 有卡就并行训练
        ...
        mp.start_processes(_distributed_worker, nprocs=num_gpu,
