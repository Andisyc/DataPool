        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter):
                ...
                if self.is_regular == True:
                    if self.batch_counter % 20 == 0: # 每10个正常batch检查一次损失值之间的差异
                        ...
                        if self.loss_sum_small / (self.loss_sum_small + self.loss_sum_medium + self.loss_sum_large) < 0.2:
                            self.batch_flag, self.is_regular, self.batch_counter, temp_iter = [1], False, 0, self.iter + self.epoch * self.max_iter
                else:
                    ...
                    if self.loss_sum_small / (self.loss_sum_small + self.loss_sum_medium + self.loss_sum_large) >= 0.2 or \
                       self.iter + self.epoch * self.max_iter - temp_iter == 5:
