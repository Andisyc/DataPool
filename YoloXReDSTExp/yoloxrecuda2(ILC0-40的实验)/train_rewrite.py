...
import sys
sys.path.append(r'/mnt/yoloxrecuda2') # r'D:\AICV-YoloXRe
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter):
                ...
                """
                if self.epoch < 40:
                    self.exp.mosaic_scale = (0.0, 0.0)
                    self.prefetcher = self.prefetcher_irregular
                    self.f_batch = self.f_batch + 1
                else:
                    self.exp.mosaic_scale = (0.1, 2.0)
                    self.prefetcher = self.prefetcher_irregular
                    self.f_batch = self.f_batch + 1
                ...
                if self.max_iter <= self.epoch * self.max_iter <= 7 * self.max_iter + 960:
                    self.batch_flag[0] == 0
