...
class VOCDetection(Dataset):
    ...
    def pull_item(self, index):
        ...
        if self.imgs is not None:
            ...
            img = self.load_resized_img(index) # 到这里为止并没有进行缩放
            target, img_info, _ = self.annotations[index]

        return img, target, img_info, index

    @Dataset.mosaic_getitem
    def __getitem__(self, index): # 这个函数不会被执行!!!真正被执行的是mosaic类中的getitem
            ...
            with open(filename, "wt") as f:
                for im_ind, index in enumerate(self.ids):
                    ...
                    for k in range(dets.shape[0]):
                        f.write("{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n".format(index, dets[k, -1], dets[k, 0] + 1, dets[k, 1] + 1,
...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx): # 真正被执行的是这个函数
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            img, label, img_info, img_id = self._dataset.pull_item(idx) # 此处是resize_img
            img, label = self.preproc(img, label, self.input_dim) # 此处是pad_img
...
class MosaicDetection_ir(Dataset):
    ...
    def __getitem__(self, idx): # 真正被执行的是这个函数
        ...
        # img, label = self.preproc(img, targets_t, self.input_dim) # 此处是pad_img channel, height width, label.shape=[120, 5]
...
class DataLoader_ir(torchDataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.batch_sampler_ir = batch_sampler_ir
