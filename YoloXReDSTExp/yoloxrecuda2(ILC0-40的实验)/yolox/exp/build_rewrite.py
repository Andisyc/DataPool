# -*- coding: utf-8 -*-
"""
Created on Thu Mar  3 17:26:35 2022
...
class Exp():
    def __init__(self):
        ...
        self.depth = 0.33 # factor of model depth
        self.width = 0.50 # factor of model width
        ...
        # --------------- transform config ----------------- #
        self.mosaic_prob = 0.0 # prob of applying mosaic aug
        ...
        self.max_epoch = 100 # max training epoch
        ...
        self.no_aug_epochs = 5 # last epoch to close augmention like mosaic
        ...
        self.seed = None
        self.traindata  = r'/mnt/yoloxredstald/datasets/'
        self.testdata = r'/mnt/yoloxredstald/datasets/'
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img=False):
        ...
        from yolox.data.dataloading_rewrite import (VOCDetection,YoloBatchSampler,DataLoader,InfiniteSampler,MosaicDetection,worker_init_reset_seed)
    ...
    def get_data_loader_ir(self, batch_size, is_distributed, no_aug=False, cache_img=False):
        
        from yolox.data import TrainTransform
        
        from yolox.data.dataloading_rewrite import (VOCDetection, YoloBatchSampler, DataLoader_ir, InfiniteSampler, MosaicDetection_ir, worker_init_reset_seed)
        ...
        dataset = MosaicDetection_ir(dataset,
                                     img_size=self.input_size,
                                     preproc=TrainTransform(max_labels=120, flip_prob=self.flip_prob, hsv_prob=self.hsv_prob))
        ...
        collator = BatchCollatorSynthesize(self.input_size, self.mosaic_scale)
        ...
        train_loader = DataLoader_ir(self.dataset, **dataloader_kwargs)
...
class BatchCollatorSynthesize(object):
    def __init__(self, input_size, mosaic_scale): # input_size=[height, width]
    ...
    def __call__(self, batch): # batch为list
        ...
        images, targets = to_image_list_synthesize_4(transposed_batch, self.input_size, self.mosaic_scale, self.size_divisible)
        
        return images, targets, 0, 0


def to_image_list_synthesize_4(transposed_info, input_size, mosaic_scale, size_divisible=0):
    ...
    if isinstance(tensors, (tuple, list)): # 判断tensors是否属于tuple或list tensors[i].shape=(3, 640, 640)
        ...
        max_size = tuple(max(s) for s in zip(*[img.shape for img in tensors])) # 取得最大图片的尺寸,(3, 428, 640) c h w
        ...
        with torch.no_grad():
            for idx, pad_img in enumerate(syn_batched_imgs): # idx = 0, pad_img.shape = [3, 320, 512], 因为len(tensors)//4==1
                ...
                new_h, new_w = max_size[1]//2, max_size[2]//2 # 缩放后尺寸是最大宽高的一半 channel height width
                
                # NOTE: interpolate api require first h then w ! interpolate()作用为缩放图片 shape=[height, width, channel]
                ...
                # 保存四合一图片进行可视化, 图片已经被填充过了 [c, h, w] 2 [h, w, c], imread reqired h, w, c
                # cv2.imwrite("D:/AICV-YoloXReDST-Orig/pad_img.jpg", np.transpose(pad_img.numpy(), (1, 2, 0)))
                
                # 保存四张子图的原始未缩放形态
                # cv2.imwrite("D:/AICV-YoloXReDST-Orig/tensor1.jpg", np.transpose(tensors[idx*4], (1, 2, 0)))
                # cv2.imwrite("D:/AICV-YoloXReDST-Orig/tensor2.jpg", np.transpose(tensors[idx*4+1], (1, 2, 0)))
                # cv2.imwrite("D:/AICV-YoloXReDST-Orig/tensor3.jpg", np.transpose(tensors[idx*4+2], (1, 2, 0)))
                # cv2.imwrite("D:/AICV-YoloXReDST-Orig/tensor4.jpg", np.transpose(tensors[idx*4+3], (1, 2, 0)))
                
                """
                # 可视化单张子图片,图片未缩放,标签未缩放,结果正确
                target1 = xywh_to_xyxy(copy.deepcopy(targets[idx*4])) # 这里会将xywh变为xyxy
                _COLORS = np.array([0.000, 0.447, 0.741]).astype(np.float32).reshape(-1, 3)
                img_tensor = cv2.imread('D:/AICV-YoloXReDST-Orig/tensor1.jpg')
                for i in range(len(target1)):
                    ...
                    cv2.rectangle(img_tensor, (x0, y0), (x1, y1), color, 2)
                cv2.imwrite("D:/AICV-YoloXReDST-Orig/tensor1.jpg", img_tensor)
                """
                
                # resize each of four sub-imgs into (new_h, new_w) scale 缩放标签, xywh2xyxy
                ...
                """
                # 可视化缩放后子图片的标签是否与目标匹配,结果正确
                cv2.imwrite("D:/AICV-YoloXReDST-Orig/topLeftImg.jpg", np.transpose(topLeftImg.numpy(), (1, 2, 0)))
                label_tensor = topLeftBL
                _COLORS = np.array([0.000, 0.447, 0.741]).astype(np.float32).reshape(-1, 3)
                img_tensor = cv2.imread('D:/AICV-YoloXReDST-Orig/topLeftImg.jpg')
                for i in range(len(label_tensor)): # [cls, x1, y1, x2, y2]
                    ...
                    color = (_COLORS[0] * 255).astype(np.uint8).tolist()
                    cv2.rectangle(img_tensor, (x0, y0), (x1, y1), color, 2)
                cv2.imwrite("D:/AICV-YoloXReDST-Orig/topLeftImg1.jpg", img_tensor)
                """
                ...
                # append offsets to box coordinates except for topLeftBL 调整GT框坐标到新位置 xyxy
                ...
                label_tensor = torch.cat((topLeftBL, topRightBL, bottomLeftBL, bottomRightBL), 0)
                _COLORS = np.array([0.000, 0.447, 0.741]).astype(np.float32).reshape(-1, 3)
                img_tensor = cv2.imread('D:/AICV-YoloXReDST-Orig/pad_img.jpg')
                for i in range(len(label_tensor)): # [cls, x1, y1, x2, y2]
                    box = label_tensor[i]
                    x0 = int(box[1])
                    y0 = int(box[2])
                    ...
                    color = (_COLORS[0] * 255).astype(np.uint8).tolist()
                    cv2.rectangle(img_tensor, (x0, y0), (x1, y1), color, 2)
                cv2.imwrite("D:/AICV-YoloXReDST-Orig/be4_pad.jpg", img_tensor)
                """
                ...
                _COLORS = np.array([0.000, 0.447, 0.741]).astype(np.float32).reshape(-1, 3)
                img_tensor = cv2.imread('D:/AICV-YoloXReDST-Orig/after_pad_img.jpg')
                for i in range(len(label_tensor)): # [cls, x1, y1, x2, y2]
                    box = label_tensor[i]
                    x0 = int(box[1])
                    y0 = int(box[2])
                    ...
                    color = (_COLORS[0] * 255).astype(np.uint8).tolist()
                    cv2.rectangle(img_tensor, (x0, y0), (x1, y1), color, 2)
                cv2.imwrite("D:/AICV-YoloXReDST-Orig/abc.jpg", img_tensor)
                """
                
                # 添加0值行变为shape=(120, 5)
                syn_bbox = torch.cat((topLeftBL, topRightBL, bottomLeftBL, bottomRightBL), dim=0)
                
                if mosaic_scale[0] != 0.0:
                    syn_bbox = syn_bbox.numpy()
                    syn_bbox = xywh_to_xyxy(syn_bbox)
                    pad_img = pad_img.numpy()
                    pad_img = np.transpose(pad_img, (1, 2, 0)) # print(pad_img.shape) == c, h, w, 需要送入的图片为h, w, c
                    pad_img, syn_bbox = random_affine(pad_img, syn_bbox, target_size=(512, 512), degrees=0.0, 
                                                      translate=0.0, scales=mosaic_scale, shear=0.0)
                    pad_img = np.transpose(pad_img, (2, 0, 1))
                    pad_img = torch.tensor(pad_img)
                    syn_bbox = xyxy_to_xywh(syn_bbox)
                    syn_bbox = torch.tensor(syn_bbox)
...
def resize(targets, sizeori, sizenew): # 输入xywh返回xyxy
    ...
    for i in range(targets.shape[0]):
        if targets[i][3]!=0 and targets[i][4]!=0:
            temp.append(copy.deepcopy(targets[i]))
    target = torch.stack(temp, dim=0)
    del temp
    ...
    for i in range(target.shape[0]):
        xmin = target[i][1] - target[i][3] / 2
        ymin = target[i][2] - target[i][4] / 2
        xmax = target[i][1] + target[i][3] / 2
        ymax = target[i][2] + target[i][4] / 2
        scaled_xmin = xmin * ratio_width
        scaled_xmax = xmax * ratio_width
        scaled_ymin = ymin * ratio_height
        scaled_ymax = ymax * ratio_height
        target[i][1] = scaled_xmin
        target[i][2] = scaled_ymin
        target[i][3] = scaled_xmax
        target[i][4] = scaled_ymax
    
    return target


def compute_tensor(tensor1, tensor2): # 专门用于为坐标添加偏移量
...
def xyxy_to_xywh(tensor): # [cls, x1, y1, x2, y2]
