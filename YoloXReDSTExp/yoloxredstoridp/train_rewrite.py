...
class Trainer:
    ...
    def train(self):
        ...
        self.train_loader_4to1 = self.exp.get_data_loader_4to1(batch_size=self.args.batch_size * 4,
                                                           is_distributed=self.is_distributed,
                                                           no_aug=self.no_aug,
                                                           cache_img=self.args.cache)
        self.train_loader_dp = self.exp.get_data_loader_dp(batch_size=self.args.batch_size,
                                                           is_distributed=self.is_distributed,
                                                           no_aug=self.no_aug,
                                                           cache_img=self.args.cache)
        logger.info("init prefetcher, this might take one minute or less...")
        self.prefetcher_regular = DataPrefetcher(self.train_loader)
        self.prefetcher_fourtoone = DataPrefetcher(self.train_loader_4to1)
        self.prefetcher_datapool = DataPrefetcher(self.train_loader_dp)
        ...
        batch_flag, is_regular, temp_iter, self.iter_normal, self.iter_four, self.iter_dp = 0, True, 0, 0, 0, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                ...
                if batch_flag == 0: # train_one_iter regular
                    ...
                    self.prefetcher = self.prefetcher_datapool
                    self.iter_dp = self.iter_dp + 1 # 非正常batch计数器加1,用于统计总非正常batch数量
                """
                
                # 分段增强, 先用致密Aug, 再用正常Aug
                if self.epoch <= 4:
                    self.prefetcher = self.prefetcher_fourtoone
                    self.iter_four = self.iter_four + 1
                else: # 已经完成初始化轮次后
                ...
                if (self.iter + 1) % self.exp.print_interval == 0:
                    ...
                    logger.info("{}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), time_str, loss_str, self.meter["lr"].latest) + \
                               (", size: {:d}, {}".format(self.input_size[0], eta_str)) + (", re_iter: {}, dp_iter: {}, f_iter:{}".format(self.iter_normal, self.iter_dp, self.iter_four)) + \
