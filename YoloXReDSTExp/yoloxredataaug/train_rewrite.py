...
import sys
sys.path.append(r'/mnt/yoloxredataaug')
...
class Trainer:
    ...
    def train(self):
        ...
        self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large, self.m_batch, self.f_batch = 1.0, 1.0, 1.0, 0, 0
        self.batch_flag, self.is_regular, self.batch_counter = [0], True, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter):
                ...
                if self.epoch <= 4:
                    ...
                    self.f_batch = self.f_batch + 1
                else:
                    self.prefetcher = self.prefetcher_regular
                    self.m_batch = self.m_batch + 1
                """
                """
                # 0epoch: ALD0.2_20_5
                if self.epoch == 0:
                    if self.is_regular == True:
                        if self.batch_counter % 20 == 0:
                            if self.loss_sum_small / (self.loss_sum_small + self.loss_sum_medium + self.loss_sum_large) < 0.2:
                                self.batch_flag, self.is_regular, self.batch_counter, temp_iter = [1], False, 0, self.iter
                    else:
                        if self.loss_sum_small / (self.loss_sum_small + self.loss_sum_medium + self.loss_sum_large) >= 0.1 or self.iter - temp_iter == 5:
                            self.batch_flag, self.is_regular = [0], True
                
                # 1 ~ 7epoch+960iter: 4to1
                if self.max_iter <= self.epoch * self.max_iter + self.iter <= 7 * self.max_iter + 960:
                    self.batch_flag[0] = 1
                
                # 7epoch+960iter ~ else: Mosaic
                if 7 * self.max_iter + 960 < self.epoch * self.max_iter + self.iter:
                    self.batch_flag[0] = 0
                ...
                if self.epoch <= 4:
                    ...
                    # 设定Mosaic与4to1的比例
                    if random.random() < self.thr_regulator():
                        self.prefetcher = self.prefetcher_regular
                        self.m_batch = self.m_batch + 1
                    else:
                        self.prefetcher = self.prefetcher_irregular
                ...
                if (self.iter + 1) % self.exp.print_interval == 0:
                    ...
                    logger.info("{}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), time_str, loss_str, self.meter["lr"].latest) + \
                               (", size: {:d}, {}".format(self.input_size[0], eta_str)) + (", m_batch: {}, f_batch: {}, s_loss: {}, m_loss: {}, l_loss: {}".format(
                                self. m_batch, self.f_batch, self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large)))
            ...
            if (self.epoch + 1) % 50 == 0 & (self.epoch + 1) > 500:
                self.save_ckpt(ckpt_name=str(self.epoch + 1))
