...
class Exp():
    def __init__(self):
        ...
        # ---------------- model config ---------------- #
        self.num_classes = 3 # detect classes number of model
        ...
        # -------------- training config --------------------- #
        self.warmup_epochs = 3 # epoch number used for warmup
        ...
        self.seed = None
        self.traindata  = r'/mnt/yoloxredstorigcodetrain/datasets_FaceMaskPerson_trainval/'
        self.testdata = r'/mnt/yoloxredstorigcodetrain/datasets_FaceMaskPerson_trainval/'
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img=False):
        ...
        dataset = MosaicDetection_bg(dataset,
                                  mosaic=not no_aug,
                                  img_size=self.input_size,
    ...
    def get_data_loader_nomix(self, batch_size, is_distributed, no_aug=False, cache_img=False):
    ...
    if isinstance(tensors, (tuple, list)): # 判断tensors是否属于tuple或list tensors[i].shape=(3, 640, 640)
        ...
        with torch.no_grad():
            for idx, pad_img in enumerate(syn_batched_imgs): # idx = 0, pad_img.shape = [3, 320, 512], 因为len(tensors)//4==1
                ...
                for i in range(200 - syn_bbox.shape[0]):
