...
class MosaicDetection_bg(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        path_bg = '/mnt/yoloxredstorigcodetrain/Background/'
        ...
        """
            ...
            path_image2 = '/mnt/yoloxredstorigcodetrain/images_2/'
            path_label = '/mnt/yoloxredstorigcodetrain/labels_2/'
            total_bg = os.listdir(path_image2)
            cp_index = random.randint(0, len(total_bg) - 1) # 随机抽取一张图片的序号
            img = cv2.imread(path_image2 + total_bg[cp_index]) # 读取图片本身
            r = min(self.img_size[0] / img.shape[0], self.img_size[1] / img.shape[1])
            img = cv2.resize(img,(int(img.shape[1] * r), int(img.shape[0] * r)),interpolation=cv2.INTER_LINEAR).astype(np.uint8)
            readinlabel = AnnotationTransform()
            cp_labels, _ = readinlabel(ET.parse(path_label + total_bg[cp_index][0:-4] + '.xml').getroot())
            height, width = _
            r = min(self.img_size[0] / height, self.img_size[1] / width)
            cp_labels[:, :4] *= r
            background = False
