...
import sys
sys.path.append(r'/mnt/yoloxredstorigcodetrain')
...
class Trainer:
    ...
    def train(self):
        ...
        self.train_loader_ir = self.exp.get_data_loader_ir(batch_size=self.args.batch_size * 4,
                                                           ...
                                                           no_aug=self.no_aug,
                                                           cache_img=self.args.cache)
        self.train_loader_nomix = self.exp.get_data_loader_nomix(batch_size=self.args.batch_size,
                                                                 is_distributed=self.is_distributed,
                                                                 no_aug=self.no_aug,
                                                                 cache_img=self.args.cache)
        ...
        self.prefetcher_nomix = DataPrefetcher(self.train_loader_nomix)
        ...
        self.iter_normal, self.iter_unnorm, self.iter_nomix = 0, 0, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            if self.epoch + 1 > 40 and self.no_aug == False:
                self.exp.eval_interval = 1
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                ...
                if ratio_scale[0] < 0.1:
                    ...
                    if random.random() > 0.95:
                        self.prefetcher = self.prefetcher_nomix
                        self.iter_nomix = self.iter_nomix + 1
                    else:
                        self.prefetcher = self.prefetcher_regular
                ...
                if (self.iter + 1) % self.exp.print_interval == 0:
                    ...
                    logger.info("{}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), time_str, loss_str, self.meter["lr"].latest) + \
                               ...
                               (", re_iter: {}, ir_iter: {}, no_mix: {}".format(self.iter_normal, self.iter_unnorm, self.iter_nomix)) + \
