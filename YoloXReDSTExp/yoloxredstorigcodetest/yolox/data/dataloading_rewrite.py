# -*- coding: utf-8 -*-
"""
Created on Mon Mar  7 20:39:11 2022
    ...
    def __init__(self,data_dir,image_sets=[("2007", "trainval"), ("2012", "trainval")],img_size=(416, 416),
        ...
        for (year, name) in image_sets:
            ...
            for line in open(os.path.join(rootpath, "ImageSets", "Main", name + ".txt")):
                self.ids.append((rootpath, line.strip())) # 读取训练集中每张图片的id
...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            print("saving mosaic img")
            cv2.imwrite('/mnt/yoloxredstorigcodetest/mosaic_' + str(idx) + '_.jpg', mosaic_img)

            # CopyPaste: https://arxiv.org/abs/2012.07177
            if (self.enable_mixup and not len(mosaic_labels) == 0 and random.random() < self.mixup_prob):
                mosaic_img, mosaic_labels = self.mixup(mosaic_img, mosaic_labels, self.input_dim)
            
            print("saving mixup img")
            cv2.imwrite('/mnt/yoloxredstorigcodetest/mixup_' + str(idx) + '_.jpg', mosaic_img)
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        cp_labels = []
        while len(cp_labels) == 0: # 注意这里一定要mixup图片上有目标
            cp_index = random.randint(0, self.__len__() - 1) # 随机抽取一张图片的序号
            cp_labels = self._dataset.load_anno(cp_index) # 读取这张图片的标签
        img, cp_labels, _, _ = self._dataset.pull_item(cp_index) # 读取图片本身
...
class MosaicDetection(Dataset):
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        path_bg = '/mnt/yoloxredstorigcodetest/Background/'
        total_bg = os.listdir(path_bg)
        
        cp_index = random.randint(0, len(total_bg) - 1) # 随机抽取一张图片的序号
        # cp_labels = self._dataset2.load_anno(cp_index) # 读取这张图片的标签
        img = cv2.imread(path_bg + total_bg[cp_index]) # 读取图片本身
...
class MosaicDetection_ir(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
...
class DataLoader_ir(torchDataLoader):
    """
    Lightnet dataloader that enables on the fly resizing of the images.
    See :class:`torch.utils.data.DataLoader` for more information on the arguments.
    Check more on the following website:
