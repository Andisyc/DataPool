...
import sys
sys.path.append(r'/mnt/yoloxredstorigcodetest')
...
class Trainer:
    ...
    def train(self):
        ...
        self.loss_sum_small, self.loss_sum_medium, self.loss_sum_large, self.iter_normal, self.iter_unnorm = 0.0, 0.0, 0.0, 0, 0
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            for self.iter in range(self.max_iter): # self.max_iter=len(self.train_loader),即数据集被划分成多少个批次
                """
                # 控制器: 0~4四合一, 4~elseMosaic+Ori四合一
                if self.epoch <= 4:
